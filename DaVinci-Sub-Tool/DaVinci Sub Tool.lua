...
Config.SCRIPT_BILIBILI_URL = "https://space.bilibili.com/385619394"
...
Config.LOADING_X = math.floor((Config.SCREEN_WIDTH - Config.LOADING_WINDOW_WIDTH) / 2)
Config.LOADING_Y = math.floor((Config.SCREEN_HEIGHT - Config.LOADING_WINDOW_HEIGHT) / 2)
...
local translatePollingTimer = nil
...
local TRANSLATE_PROVIDER_SPECS = {
    ...
    { label = TRANSLATE_PROVIDER_DEEPL_LABEL, kind = "deepl", langKey = "deepl", smartMergeMode = "ratio" },
}
local TRANSLATE_PROVIDER_INDEX = {}
local TRANSLATE_PROVIDER_LIST = {}
for _, spec in ipairs(TRANSLATE_PROVIDER_SPECS) do
    TRANSLATE_PROVIDER_INDEX[spec.label] = spec
    TRANSLATE_PROVIDER_LIST[#TRANSLATE_PROVIDER_LIST + 1] = spec.label
end

function Translate.getProviderSpec(label)
    return TRANSLATE_PROVIDER_INDEX[label]
end

function Translate.isSupportedProvider(label)
    return Translate.getProviderSpec(label) ~= nil
end
...
local TRANSLATE_TIMEOUT = 10
local SMART_MERGE_TIMEOUT = 120
...
local AZURE_TIMEOUT = TRANSLATE_TIMEOUT
...
local GLM_TIMEOUT = TRANSLATE_TIMEOUT
...
local OPENAI_FORMAT_TIMEOUT = TRANSLATE_TIMEOUT
...
local SMART_MERGE_MAX_CHUNKS = 5
local SMART_MERGE_SENTENCE_PUNCT = "[%.%!%?。！？；;，,]$"
local SMART_MERGE_PREFIX_PROMPT = [[You are a subtitle translation + segmentation engine.
...
local uiText = {
    cn = {
        ...
        copyright = " © 2025, 版权所有 " .. Config.SCRIPT_AUTHOR,
    ...
    en = {
        ...
        copyright = " © 2025, copyright by " .. Config.SCRIPT_AUTHOR,
        ...
        translate_smart_merge = "Semantic Boost",
...
local translateStatusTemplates = {
    ...
    preparing = { cn = "正在准备翻译...", en = "Preparing translation..." },
...
local translateErrorMessages = {
    ...
    timeout = { cn = "请求超时", en = "Request timed out." },
...
local runHiddenWindowsCommand, runHiddenWindowsCommandAsync, runHiddenWindowsCommandAsyncRaw, runShellCommand
local winfs
do
...
------------------------------------------------------------------
-- AsyncTranslation: non-blocking curl + polling
------------------------------------------------------------------
local AsyncTranslation = {}
App.AsyncTranslation = AsyncTranslation
AsyncTranslation.debug = false

AsyncTranslation.state = {
    active = false,
    artifacts = {},
    pendingCount = 0,
    completedCount = 0,
...
function AsyncTranslation.isActive()
    return AsyncTranslation.state.active == true
end

function AsyncTranslation.resetState()
    local s = AsyncTranslation.state
    s.active = false
    s.artifacts = {}
    s.pendingCount = 0
    s.completedCount = 0
...
function AsyncTranslation.cleanupArtifacts(artifacts)
    for _, art in ipairs(artifacts or {}) do
        if art.output then
...
function AsyncTranslation.ensureTimer()
    if translatePollingTimer then
        return translatePollingTimer
    end
    translatePollingTimer = ui:Timer{
...
function AsyncTranslation.startPollingTimer()
    local timer = AsyncTranslation.ensureTimer()
    if timer then
        pcall(function() timer:Start() end)
    end
end

function AsyncTranslation.stopPollingTimer()
    if translatePollingTimer then
        pcall(function() translatePollingTimer:Stop() end)
    end
end

function AsyncTranslation.spawnBackgroundCommand(command)
    if IS_WINDOWS and type(runHiddenWindowsCommandAsync) == "function" then
        local ok, code
        if type(runHiddenWindowsCommandAsyncRaw) == "function" then
            ok, code = runHiddenWindowsCommandAsyncRaw(command)
        else
            ok, code = runHiddenWindowsCommandAsync(command)
        end
        if not ok then
    ...
    local handle = io.popen(command .. " >/dev/null 2>&1 &", "r")
    if handle then
        handle:close()
        return true, 0
    end
...
function AsyncTranslation.buildSingleCurlCommand(task, payloadPath, outputPath, options)
    local headers = options.headers or {}
    local timeout = options.timeout or GLM_TIMEOUT
    local apiUrl = task.url or options.apiUrl or GLM_API_URL
    local parts = { "curl", "-sS", "--show-error", string.format("-m %d", timeout) }
    table.insert(parts, "-X")
    table.insert(parts, "POST")
    for _, header in ipairs(headers) do
        table.insert(parts, "-H")
        table.insert(parts, string.format("%q", header))
    end
    table.insert(parts, "--data-binary")
    table.insert(parts, string.format("@%q", payloadPath))
    table.insert(parts, "-o")
    table.insert(parts, string.format("%q", outputPath))
    table.insert(parts, string.format("%q", apiUrl))
    return table.concat(parts, " ")
end

function AsyncTranslation.launchRetry(artifact, options)
    if not artifact or not artifact.payload then
        return false, "invalid_artifact"
    end
    if artifact.output then
        os.remove(artifact.output)
    end
    local command = AsyncTranslation.buildSingleCurlCommand(artifact.task or {}, artifact.payload, artifact.output, options or {})
    return AsyncTranslation.spawnBackgroundCommand(command)
end

function AsyncTranslation.launchBackgroundCurl(tasks, options)
    if AsyncTranslation.state.active then
    ...
    local function cleanupArtifacts()
        AsyncTranslation.cleanupArtifacts(artifacts)
    end
    ...
    for idx, task in ipairs(tasks) do
        ...
        if not okWrite then
            cleanupArtifacts()
            return false, string.format("payload_tmp_open_failed: %s", tostring(err))
        end
        local url = task.url or apiUrl
        ...
        table.insert(commandParts, "-o")
        table.insert(commandParts, string.format("%q", outputPath))
        table.insert(commandParts, string.format("%q", url))
        if idx < #tasks then
    ...
    local command = table.concat(commandParts, " ")
    local ok = AsyncTranslation.spawnBackgroundCommand(command)
    if not ok then
        cleanupArtifacts()
    ...
    s.pendingCount = #artifacts
    s.completedCount = 0
    s.parser = parser
    s.onComplete = options.onComplete
    s.onProgress = options.onProgress
    s.apiUrl = apiUrl
    s.headers = headers
    s.timeout = timeout
    s.parallelLimit = limit
    s.payloadPrefix = payloadPrefix
    s.outputPrefix = outputPrefix
    s.maxRetry = tonumber(options.maxRetry) or 0
    s.readRetry = tonumber(options.readRetry) or 2
    s.pollTimeout = tonumber(options.pollTimeout) or math.max(5, (tonumber(timeout) or GLM_TIMEOUT) + 5)

    AsyncTranslation.startPollingTimer()
    return true
end

function AsyncTranslation.finish()
    local s = AsyncTranslation.state
    local onComplete = s.onComplete
    local artifacts = s.artifacts
    AsyncTranslation.stopPollingTimer()
    AsyncTranslation.resetState()
    AsyncTranslation.cleanupArtifacts(artifacts)
...
function AsyncTranslation.pollOutputFiles()
    local s = AsyncTranslation.state
    if not s.active then
        return
    end
    local now = os.time()
    local pollTimeout = tonumber(s.pollTimeout) or 0

    local function markTimeout(art)
        art.completed = true
        s.completedCount = s.completedCount + 1
        AsyncTranslation.cleanupArtifacts({ art })
        local result = {
            success = false,
            err = "timeout",
        }
        if type(s.onProgress) == "function" then
            local ok, err = pcall(s.onProgress, art.task or {}, result)
            if not ok then
                print(string.format("[AsyncTranslation] onProgress error (timeout): %s", tostring(err)))
            end
        end
    end

    local function emitProgress(task, result, label)
        if type(s.onProgress) ~= "function" then
            return
        end
        local ok, err = pcall(s.onProgress, task or {}, result)
        if not ok then
            print(string.format("[AsyncTranslation] onProgress error (%s): %s", tostring(label or "progress"), tostring(err)))
        end
    end

    local function handleParseResult(art, translation, tokens, errMsg)
        if translation then
            art.completed = true
            s.completedCount = s.completedCount + 1
            AsyncTranslation.cleanupArtifacts({ art })
            ...
            emitProgress(art.task, result, "success")
            return
        end

        art.readAttempts = (art.readAttempts or 0) + 1
        local shouldWait = (errMsg == "decode_failed" or errMsg == "invalid_response" or errMsg == "empty_response")
        if (art.readAttempts or 0) <= s.readRetry and shouldWait then
            return
        end

        art.attempts = (art.attempts or 0) + 1
        if s.maxRetry > 0 and (art.attempts or 0) < s.maxRetry then
            art.readAttempts = 0
            art.lastSize = nil
            local okRetry = AsyncTranslation.launchRetry(art, s)
            if not okRetry then
                art.completed = true
                s.completedCount = s.completedCount + 1
                AsyncTranslation.cleanupArtifacts({ art })
                local result = {
                    success = false,
                    err = "parallel_execution_failed",
                }
                emitProgress(art.task, result, "retry_failed")
            end
        else
            art.completed = true
            s.completedCount = s.completedCount + 1
            ...
            emitProgress(art.task, result, "failed")
        end
    end

    for _, art in ipairs(s.artifacts or {}) do
        if not art.completed then
            if pollTimeout > 0 and art.startedAt and (now - art.startedAt) >= pollTimeout then
                markTimeout(art)
            else
                local outputPath = art.output
                if outputPath and Utils.fileExists(outputPath) then
                    local size = Utils.getFileSize(outputPath) or 0
                    if size > 0 then
                        if art.lastSize and art.lastSize == size then
                            local content = Utils.readFile(outputPath)
                            if content and content ~= "" then
                                local translation, tokens, errMsg = s.parser(content)
                                handleParseResult(art, translation, tokens, errMsg)
                            else
                                handleParseResult(art, nil, 0, "empty_response")
                            end
                        else
                            art.lastSize = size
                            art.readAttempts = 0
    ...
    if s.completedCount >= s.pendingCount then
        AsyncTranslation.finish()
    end
end

function AsyncTranslation.onTimeout(ev)
    if not AsyncTranslation.state.active then
        AsyncTranslation.stopPollingTimer()
        return true
    end
    local ok, err = pcall(AsyncTranslation.pollOutputFiles)
    if not ok then
        print(string.format("[AsyncTranslation] pollOutputFiles error: %s", tostring(err)))
    end
    if AsyncTranslation.state.active then
        AsyncTranslation.startPollingTimer()
    end
...
function Subtitle._gcd(a, b)
    a, b = math.abs(a), math.abs(b)
    while b ~= 0 do a, b = b, a % b end
    return a
end

function Subtitle._normalize(num, den)
    if den == 0 then return 0, 1 end
    if den < 0 then num, den = -num, -den end
    local g = Subtitle._gcd(num, den)
...
function Subtitle.fpsToFraction(v)
    if type(v) == "table" and v.num and v.den then
        local n, d = Subtitle._normalize(tonumber(v.num) or 0, tonumber(v.den) or 1)
    ...
    if type(v) == "string" then
        ...
        if a and b then
            local n, d = Subtitle._normalize(tonumber(a), tonumber(b))
            return { num = n, den = d }
        end
        -- 别名：常见 NTSC 比率
...
function UI.ensureSponsorQrs()
    ...
    if not Utils.fileExists(paypalPath) then
        local ok = Utils.createImageFromBase64(Config.QR_PAYPAL_BASE64, paypalPath)
        if not ok then
            return nil, "create_paypal_qr_failed"
        end
    end
    if not Utils.fileExists(wechatPath) then
        local ok = Utils.createImageFromBase64(Config.QR_WECHAT_BASE64, wechatPath)
        if not ok then
...
function UI.runWithLoading(action)
    ...
    local loadingWin = disp:AddWindow({
        ID = "LoadingWindow",
        WindowTitle = string.format("%s Loading", Config.SCRIPT_NAME),
        Geometry = { Config.LOADING_X, Config.LOADING_Y, Config.LOADING_WINDOW_WIDTH, Config.LOADING_WINDOW_HEIGHT },
    }, ui:VGroup{
        ID = "LoadingRoot",
        Weight = 1,
        ui:Label{
...
function Translate.getLangMapKey(provider)
    local spec = Translate.getProviderSpec(provider)
    if not spec or not spec.langKey then
        error(string.format("Unsupported provider for language map: %s", tostring(provider)))
    end
    return spec.langKey
end

function Translate.getProviderCodes(provider)
    local key = Translate.getLangMapKey(provider)
...
function Translate.getPopularCodes(provider)
    local key = Translate.getLangMapKey(provider)
...
function Translate.getLabelMap(provider, lang)
    local key = Translate.getLangMapKey(provider)
...
function Translate.buildOrderedTargetCodes(provider)
    local providerCodes = Translate.getProviderCodes(provider)
    local popularCodes = Translate.getPopularCodes(provider)
    local ordered = {}
    local seen = {}
    local providerSet = {}
...
function Translate.getTargetCodeFromIndex(provider, index)
    local codes = state.translate and state.translate.targetCodes
    if type(codes) ~= "table" or #codes == 0 then
        codes = Translate.buildOrderedTargetCodes(provider)
...
function Translate.getLangOptions(provider)
    ...
    local codes = Translate.buildOrderedTargetCodes(useProvider)
    local labelMap = Translate.getLabelMap(useProvider, langKey)
    local labels = {}
...
if storedSettings then
    ...
    local smartMerge = storedSettings.TranslateSmartMergeCheck
    if type(smartMerge) == "boolean" then
        state.translate.smartMerge = smartMerge
    end
    ...
    ui:Stack{
      ...
      ui:VGroup{
        ...
        ui:HGroup{
          ...
          ui:CheckBox{ ID = "TranslateSmartMergeCheck", Text = UI.uiString("translate_smart_merge"), Checked = false, Weight = 0 },
        },
...
local azureConfigWin = disp:AddWindow(
  {
    ...
    Geometry = { Config.X_CENTER + 30, Config.Y_CENTER + 30, 300, 150 },
    Hidden = true,
    StyleSheet = "*{font-size:14px;}",
  },
  ui:VGroup{
...
local deeplConfigWin = disp:AddWindow(
  {
    ...
    Geometry = { Config.X_CENTER + 35, Config.Y_CENTER + 35, 300, 130 },
    Hidden = true,
    StyleSheet = "*{font-size:14px;}",
  },
  ui:VGroup{
...
local openAIConfigWin = disp:AddWindow(
  {
    ...
    Geometry = { Config.X_CENTER + 40, Config.Y_CENTER + 40, 350, 450 },
    Hidden = true,
    StyleSheet = "*{font-size:14px;}",
  },
  ui:VGroup{
...
local addModelWin = disp:AddWindow(
  {
    ...
    Geometry = { Config.X_CENTER + 60, Config.Y_CENTER + 60, 300, 200 },
    Hidden = true,
    StyleSheet = "*{font-size:14px;}",
  },
  ui:VGroup{
...
local messageWin = disp:AddWindow(
  {
    ID = "MessageBoxWin",
    WindowTitle = "Info",
    Geometry = { Config.X_CENTER + 60, Config.Y_CENTER + 80, 360, 160 },
    Hidden = true,
    StyleSheet = "*{font-size:14px;}",
  },
  ui:VGroup{
...
function Translate.findConcurrencyOptionIndexByValue(value)
...
function Translate.setConcurrencyComboSelection(value)
    if not it.TranslateConcurrencyCombo then
        return
    end
    local idx, matchedValue = Translate.findConcurrencyOptionIndexByValue(value)
...
function Translate.populateConcurrencyCombo(value)
    if not it.TranslateConcurrencyCombo then
        return
    end
    ...
    Translate.setConcurrencyComboSelection(value)
end
...
function UI.getCurrentTimelineFrame()
...
function UI.findEntryIndexByFrame(absFrame)
...
function UI.followPlayheadTick()
    ...
    local frame = UI.getCurrentTimelineFrame()
    if not frame then
        return
    end
    ...
    local index = UI.findEntryIndexByFrame(frame)
    if not index then
...
function UI.updatePlayheadTimerState(triggerTick)
...
function disp.On.Timeout(ev)
    ...
    if who == "TranslatePollingTimer" then
        if AsyncTranslation and AsyncTranslation.onTimeout then
            return AsyncTranslation.onTimeout(ev)
        end
        return true
    end
...
function UI.showMessage(en_text, zh_text)
...
function Translate.formatTranslateStatusForLang(key, args, lang)
...
function Translate.formatTranslateStatus(key, args)
    return Translate.formatTranslateStatusForLang(key, args, UI.currentLanguage())
end

function Translate.notifyTranslateStatus(key, argsEn, argsCn)
    argsEn = argsEn or {}
    argsCn = argsCn or argsEn
    local enText = Translate.formatTranslateStatusForLang(key, argsEn, "en")
    local cnText = Translate.formatTranslateStatusForLang(key, argsCn, "cn")
    if enText == "" and cnText == "" then
        return
    end
    local finalEn = enText ~= "" and enText or cnText
    local finalCn = cnText ~= "" and cnText or enText
    UI.showMessage(finalEn, finalCn)
end

function Translate.applyTranslateStatusInternal(key, args)
    if it.TranslateStatusLabel then
        it.TranslateStatusLabel.Text = Translate.formatTranslateStatus(key, args)
    end
end

function Translate.setStatus(key, ...)
    local args = { ... }
    ...
    Translate.applyTranslateStatusInternal(key, args)
end

function Translate.computeTranslateSummary(entries)
...
function Translate.applyTranslateSuccessStatus(entries, tokenCount, shouldNotify)
    local total, success, failed = Translate.computeTranslateSummary(entries)
    local tokens = tokenCount or 0
    Translate.setStatus("success", total, success, failed, tokens)
    if shouldNotify then
        Translate.notifyTranslateStatus("success", { total, success, failed, tokens }, { total, success, failed, tokens })
    end
    return total, success, failed
end

function Translate.resolveTranslateErrorForLang(code, lang)
...
function Translate.resolveError(code)
    return Translate.resolveTranslateErrorForLang(code, UI.currentLanguage())
end

function Translate.resolveErrorPair(code)
    local enText = Translate.resolveTranslateErrorForLang(code, "en")
    local cnText = Translate.resolveTranslateErrorForLang(code, "cn")
    return enText, cnText
end

function Translate.normalizeErrorCode(code, provider)
    local normalized = Utils.trim(tostring(code or ""))
    if normalized == "" then
        normalized = "translation_failed"
    end
    local spec = Translate.getProviderSpec and Translate.getProviderSpec(provider)
    if normalized == "parallel_execution_failed" and spec and (spec.kind == "openai" or spec.kind == "chat") then
        normalized = "openai_parallel_failed"
    end
    return normalized
end

function Translate.normalizeError(err, fallbackCode, provider)
    if type(err) == "table" then
        local code = err.code or err.err or err.reason or err[1]
        local detail = err.detail or err.message or err.msg
        local normalizedCode = Translate.normalizeErrorCode(code or fallbackCode, provider)
        if (not detail or detail == "") and type(err) == "table" then
            detail = err.detail or err.message or err.msg
        end
        return {
            code = normalizedCode,
            detail = detail,
    ...
    if type(err) == "string" then
        local head, tail = err:match("^([%w_]+)%s*:%s*(.+)$")
...
function Translate.formatTranslateFailure(err, provider)
    local normalized = Translate.normalizeError(err, "translation_failed", provider)
    local label = normalized.code or "translation_failed"
    if normalized.detail and normalized.detail ~= "" then
        label = string.format("%s: %s", label, tostring(normalized.detail))
    end
    return string.format("[Error: %s]", tostring(label))
end

function Translate.reportError(err, opts)
    opts = opts or {}
    local normalized = Translate.normalizeError(err, opts.fallbackCode or "translation_failed", opts.provider)
    if opts.setStatus ~= false then
        Translate.setStatus("failed", Translate.resolveError(normalized.code))
    end
    if opts.showMessage then
        local enText, cnText = Translate.resolveErrorPair(normalized.code)
        UI.showMessage(enText, cnText)
    end
    if opts.notify then
        Translate.notifyFailure(normalized.code)
    end
    return normalized
end

function Translate.notifyFailure(reason)
    if reason == nil then
        Translate.notifyTranslateStatus("failed", { "" }, { "" })
        return
    end
    if type(reason) == "table" then
        local enReason = reason.en or reason[1] or ""
        local cnReason = reason.cn or reason[2] or enReason
        Translate.notifyTranslateStatus("failed", { enReason }, { cnReason })
        return
    end
    local enReason, cnReason = Translate.resolveErrorPair(reason)
    Translate.notifyTranslateStatus("failed", { enReason }, { cnReason })
end

function Translate.handleVerify()
    if not openAIConfigItems then
        return
    end
    OpenAIService.applyConfigFromControls()
    ...
    if not selected then
        Translate.setStatus("failed", Translate.resolveError("openai_missing_model"))
        local enText, cnText = Translate.resolveErrorPair("openai_missing_model")
        UI.showMessage(enText, cnText)
        return
    end
    ...
    if apiKey == "" then
        Translate.setStatus("failed", Translate.resolveError("openai_missing_key"))
        local enText, cnText = Translate.resolveErrorPair("openai_missing_key")
        UI.showMessage(enText, cnText)
        return
    end
    ...
    if ok then
        ...
        Translate.setStatus("failed", statusText)
        UI.showMessage(errMsgEn, errMsgCn)
        print(string.format("[OpenAI] Verify failed: %s", errMsgEn))
    end
end

function Translate.handleAddModel()
    ...
    if modelInput == "" then
        Translate.setStatus("failed", UI.uiString("openai_missing_fields"))
        return
    end
    ...
    if openAIModelStore.builtin and openAIModelStore.builtin[displayInput] then
        Translate.setStatus("failed", UI.uiString("openai_add_duplicate"))
        return
    end
    Storage.ensureOpenAIModelList(state.openaiFormat)
...
function Translate.handleDeleteModel()
    OpenAIService.applyConfigFromControls()
    Storage.ensureOpenAIModelList(state.openaiFormat)
    local models = state.openaiFormat.models or {}
    if #models == 0 then
...
function Translate.refreshTranslateStatus()
    local tState = state.translate
    if not tState then
        return
    end
    Translate.applyTranslateStatusInternal(tState.lastStatusKey or "idle", tState.lastStatusArgs or {})
end

function Translate.hasFailedTranslateEntries()
...
function Translate.updateRetryButtonState()
    ...
    it.TranslateRetryFailedButton.Enabled = Translate.hasFailedTranslateEntries()
end

function Translate.setControlsEnabled(enabled)
    ...
    if it.TranslateSmartMergeCheck then
        it.TranslateSmartMergeCheck.Enabled = flag
    end
...
function Translate.normalizeTranslateTree()
    ...
    UI.withUpdatesSuspended(it.TranslateSubtitleTree, function()
        ...
        it.TranslateSubtitleTree.ColumnWidth[2] = 150
        it.TranslateSubtitleTree.ColumnWidth[3] = 260
    end)
end
function Translate.clearAllTranslateFailures()
    if not state.translate.failed then
        return
    end
    for idx in pairs(state.translate.failed) do
        Translate.setTranslateRowHighlight(idx, nil)
    end
    state.translate.failed = {}
    Translate.updateRetryButtonState()
end
function Translate.resetState()
    local tState = state.translate
    if not tState then
        return
    end
    ...
    Translate.clearAllTranslateFailures()
    tState.failed = {}
...
function Translate.cloneEditEntriesForTranslate()
    local cloned = {}
...
function Translate.populateTranslateTree()
    Translate.normalizeTranslateTree()
    if not it.TranslateSubtitleTree then
        return
    end
    ...
    UI.withUpdatesSuspended(it.TranslateSubtitleTree, function()
        ...
        for index, entry in ipairs(entries) do
            ...
            if state.translate.failed and state.translate.failed[index] then
                Translate.setTranslateRowHighlight(index, translateFailureColor)
            end
        end
    end)
    Translate.updateRetryButtonState()
...
function Translate.updateTranslateTreeRow(index, text)
    if not it.TranslateSubtitleTree then
        return
    end
    UI.withUpdatesSuspended(it.TranslateSubtitleTree, function()
...
function Translate.setTranslateRowHighlight(index, color)
    if not it.TranslateSubtitleTree then
        return
    end
    local item = it.TranslateSubtitleTree:TopLevelItem(index - 1)
...
function Translate.markTranslateFailure(index, reason)
    if not index then
        return
    end
    local tState = state.translate
...
function Translate.clearTranslateFailure(index)
    if not index then
        return
    end
    local tState = state.translate
    ...
    Translate.setTranslateRowHighlight(index, nil)
    Translate.updateRetryButtonState()
end

function Translate.updateTranslateOriginalRow(index, text)
    if not it.TranslateSubtitleTree then
        return
    end
    UI.withUpdatesSuspended(it.TranslateSubtitleTree, function()
...
function Translate.ensureTranslateEntries(force)
    local tState = state.translate
    if not tState then
        return
    end
    ...
    Translate.setStatus("copying")
    tState.entries = Translate.cloneEditEntriesForTranslate()
    tState.populated = true
    tState.selectedIndex = nil
    tState.totalTokens = 0
    Translate.clearAllTranslateFailures()
    tState.failed = {}
    Translate.populateTranslateTree()
    if it.TranslateSubtitleEditor then
        UI.withTranslateProgrammatic(function()
            it.TranslateSubtitleEditor.Text = ""
        end)
...
function Translate.refreshTranslateTargetCombo(provider, preferredLabel, preferredCode)
...
function Translate.resolveTargetSelection(provider, targetLabel)
    ...
    if not code or code == "" then
        ...
        code = Translate.getTargetCodeFromIndex(provider, index)
    end
    if not code or code == "" then
        for _, value in ipairs(codes) do
...
function Translate.resolveTranslateRules()
...
function Translate.composeTranslatePrompt(targetLabel)
    ...
    local corePrompt = Translate.resolveTranslateRules()
    return table.concat({ prefix, corePrompt, TRANSLATE_SUFFIX_PROMPT }, "\n")
end

function Translate.fetch_provider_secret(provider)
...
function Translate.buildTranslateContext(entries, index)
...
function Translate.utf8CodepointAt(text, index)
    local c1 = text:byte(index)
    if not c1 then
        return nil
    end
    if c1 < 0x80 then
    ...
    local c2 = text:byte(index + 1)
    if c1 < 0xE0 then
        if not c2 then return nil end
        return ((c1 % 0x20) * 0x40) + (c2 % 0x40), index + 2
    end
    local c3 = text:byte(index + 2)
    if c1 < 0xF0 then
        if not c2 or not c3 then return nil end
        return ((c1 % 0x10) * 0x1000) + ((c2 % 0x40) * 0x40) + (c3 % 0x40), index + 3
    end
    local c4 = text:byte(index + 3)
    if c1 < 0xF8 then
        if not c2 or not c3 or not c4 then return nil end
        return ((c1 % 0x08) * 0x40000) + ((c2 % 0x40) * 0x1000) + ((c3 % 0x40) * 0x40) + (c4 % 0x40), index + 4
    end
    return nil
end

function Translate.buildCodepointSet(text)
    local set = {}
    local i = 1
    while i <= #text do
        local code, nextIndex = Translate.utf8CodepointAt(text, i)
...
local SMART_MERGE_PUNCT_SET = Translate.buildCodepointSet(".,;:!?，。；：！？、·…—\"'()[]{}")

function Translate.isCjkChar(code)
...
function Translate.isCjkText(text)
    ...
    while i <= #text do
        ...
        if not code then break end
        total = total + 1
        if Translate.isCjkChar(code) then
            cjk = cjk + 1
        end
        i = nextIndex
    end
...
function Translate.isPunctOnly(text)
    if not text or text == "" then
        return true
    end
    local i = 1
    while i <= #text do
        local code, nextIndex = Translate.utf8CodepointAt(text, i)
        if not code then break end
        if code ~= 0x20 and code ~= 0x09 and code ~= 0x0A and code ~= 0x0D then
            if not SMART_MERGE_PUNCT_SET[code] then
                return false
            end
        end
        i = nextIndex
...
function Translate.splitCjkUnits(text)
    local units = {}
    local i = 1
    while i <= #text do
        ...
        if byte and byte < 0x80 then
            ...
            local _, nextIndex = Translate.utf8CodepointAt(text, i)
            if not nextIndex then break end
            units[#units + 1] = text:sub(i, nextIndex - 1)
            i = nextIndex
        end
    end
    return units
...
function Translate.redistributeEmptySegments(segments, isCjk)
    if #segments <= 1 then
        return segments
    end
    local result = {}
    local nonEmptyIndices = {}
    for i, seg in ipairs(segments) do
        local trimmed = Utils.trim(seg or "")
        result[i] = trimmed
        if trimmed ~= "" and not Translate.isPunctOnly(trimmed) then
            nonEmptyIndices[#nonEmptyIndices + 1] = i
        end
    end
    if #nonEmptyIndices == 0 then
        local combined = isCjk and table.concat(result, "") or table.concat(result, " ")
        local output = { Utils.trim(combined) }
        for i = 2, #result do
            output[i] = ""
        end
        return output
    ...
    for i, seg in ipairs(result) do
        if seg == "" or Translate.isPunctOnly(seg) then
            local donorIdx = nonEmptyIndices[1]
            local best = math.abs(donorIdx - i)
            for _, idx in ipairs(nonEmptyIndices) do
                local dist = math.abs(idx - i)
                if dist < best then
                    best = dist
                    donorIdx = idx
                end
            end
            local donor = result[donorIdx]
            if isCjk then
                local units = Translate.splitCjkUnits(donor)
                if #units > 1 then
                    local splitPt = math.max(1, math.floor(#units / 2))
                    if i < donorIdx then
                        result[i] = table.concat(units, "", 1, splitPt)
                        result[donorIdx] = table.concat(units, "", splitPt + 1)
                    else
                        result[i] = table.concat(units, "", splitPt + 1)
                        result[donorIdx] = table.concat(units, "", 1, splitPt)
                    end
                end
            else
                local words = {}
                for word in donor:gmatch("%S+") do
                    words[#words + 1] = word
                end
                if #words > 1 then
                    local splitPt = math.max(1, math.floor(#words / 2))
                    if i < donorIdx then
                        result[i] = table.concat(words, " ", 1, splitPt)
                        result[donorIdx] = table.concat(words, " ", splitPt + 1)
                    else
                        result[i] = table.concat(words, " ", splitPt + 1)
                        result[donorIdx] = table.concat(words, " ", 1, splitPt)
                    end
                end
            end
        end
...
function Translate.splitTranslationByRatio(translation, sources)
    local n = #sources
    if n == 0 then
        return {}
    end
    if n == 1 then
        return { Utils.trim(translation or "") }
    end
    translation = Utils.trim(translation or "")
    if translation == "" then
        local empty = {}
        for i = 1, n do
            empty[i] = ""
    ...
    local cjk = Translate.isCjkText(translation)
    local units = {}
    if cjk then
        units = Translate.splitCjkUnits(translation)
    else
        for word in translation:gmatch("%S+") do
            units[#units + 1] = word
        end
    end
    if #units == 0 then
    ...
    local baseCount = math.max(1, math.floor(totalUnits / n))
    local result = {}
    local idx = 1
    for i = 1, n do
        local segmentUnits
        if i == n then
            segmentUnits = {}
            for j = idx, totalUnits do
                segmentUnits[#segmentUnits + 1] = units[j]
            end
        else
            local endIdx = math.min(idx + baseCount - 1, totalUnits)
            if endIdx < idx then
                endIdx = idx
            end
            segmentUnits = {}
        ...
        if cjk then
            result[i] = table.concat(segmentUnits, "")
        else
            result[i] = table.concat(segmentUnits, " ")
        end
    end
    return Translate.redistributeEmptySegments(result, cjk)
end

function Translate.mergeSubtitleChunks(entries, indices)
    local groups = {}
    local current = { indices = {}, sources = {} }
    local lastIndex = nil
    table.sort(indices)
    for _, idx in ipairs(indices) do
        local entry = entries[idx]
        local source = Utils.trim(entry and entry.original or "")
        if #current.indices > 0 and lastIndex and idx ~= lastIndex + 1 then
            current.chunkCount = #current.indices
            groups[#groups + 1] = current
            current = { indices = {}, sources = {} }
        end
        current.indices[#current.indices + 1] = idx
        current.sources[#current.sources + 1] = source
        lastIndex = idx
        if #current.indices >= SMART_MERGE_MAX_CHUNKS or (source ~= "" and source:match(SMART_MERGE_SENTENCE_PUNCT)) then
            current.chunkCount = #current.indices
            groups[#groups + 1] = current
            current = { indices = {}, sources = {} }
            lastIndex = nil
...
function Translate.parseSmartMergeJson(response, chunkCount)
    if type(response) ~= "string" or response == "" then
        return nil, "empty_translation"
    end
    local jsonStr = response:match("{.-\"translations\".-}")
    if not jsonStr then
        return nil, "invalid_response"
    end
    local decoded, _, err = json.decode(jsonStr)
...
function Translate.buildSmartMergePrompt(targetLabel, chunkCount)
    local prompt = SMART_MERGE_PREFIX_PROMPT
    prompt = prompt:gsub("{target_lang}", function() return targetLabel or "" end)
    prompt = prompt:gsub("{chunk_count}", function() return tostring(chunkCount or 0) end)
    prompt = prompt:gsub("{translate_rules}", function() return Translate.resolveTranslateRules() end)
    return prompt
end

function Translate.buildSmartMergeMessages(targetLabel, sources)
    local prompt = Translate.buildSmartMergePrompt(targetLabel, #sources)
    return {
        { role = "system", content = prompt },
        { role = "user", content = json.encode({ segments = sources }) },
    }
end
...
function ChatProviders.getConfig(configOrId)
...
function ChatProviders.get(id)
    return ChatProviders.getConfig(id)
end
...
function ChatProviders.buildMessages(sentence, prefixText, suffixText, targetLabel)
    local prompt = Translate.composeTranslatePrompt(targetLabel)
    local messages = {
        { role = "system", content = prompt },
    }
    local ctxParts = {}
...
function ChatProviders.resolveApiKey(config)
    ...
    if config.supabaseProvider and config.supabaseProvider ~= "" then
        return Translate.fetch_provider_secret(config.supabaseProvider)
    end
    return nil, "missing_key"
end
...
function ChatProviders.buildSmartMergePayload(configOrId, sources, targetLabel)
    ...
    local payload = {
        model = config.model,
        messages = Translate.buildSmartMergeMessages(targetLabel, sources),
        temperature = config.temperature or OPENAI_FORMAT_DEFAULT_TEMPERATURE,
    }
    if type(config.payloadExtras) == "table" then
        for key, value in pairs(config.payloadExtras) do
            payload[key] = value
    ...
    if type(config.preparePayload) == "function" then
        local prepared = config.preparePayload(payload, table.concat(sources or {}, ""), "", "", targetLabel)
        if prepared ~= nil then
            payload = prepared
        end
    end
...
function Translate.applyEntryResult(task, result, providerLabel)
    local entry = task.entry
    local rowIndex = task.rowIndex or (entry and entry.index) or task.index
    if result and result.success then
        local translation = result.translation or ""
        if entry then
            entry.translation = translation
        end
        Translate.updateTranslateTreeRow(rowIndex, translation)
        Translate.clearTranslateFailure(rowIndex)
        return
    end
    local normalized = Translate.normalizeError(result and result.err, "translation_failed", providerLabel)
    local fallback = Translate.formatTranslateFailure(normalized, providerLabel)
    if entry then
        entry.translation = fallback
    end
    Translate.updateTranslateTreeRow(rowIndex, fallback)
    Translate.markTranslateFailure(rowIndex, normalized.code)
end

function Translate.runBatchRequests(options)
    if not options then
    ...
    if not list and entries then
        list = Translate.buildIndexList(entries, options.indices)
    end
    if not list or #list == 0 then
        return nil, "no_entries"
    end

    local total = options.totalCount or #list
    local concurrency = math.max(1, options.concurrency or (state.translate and state.translate.concurrency or DEFAULT_TRANSLATE_CONCURRENCY))
    if options.allowParallel == false then
        concurrency = 1
    end
    local tokenBase = tonumber(options.tokenBase) or 0
    local extraOnComplete = options.onComplete
    local summaryEntries = options.summaryEntries or entries
    local apiUrl = options.apiUrl
    if not apiUrl or apiUrl == "" then
        return nil, "invalid_api_url"
    end
    local timeout = options.timeout
    local headers = options.headers
    local payloadPrefix = options.payloadPrefix
    local outputPrefix = options.outputPrefix
    local parser = options.parseResponse
    local maxRetry = options.maxRetry
    local buildTask = options.buildTask
    local onTaskResult = options.onTaskResult
    if type(buildTask) ~= "function" then
        return nil, "payload_build_failed"
    end

    local processed = 0
    local totalTokens = 0
    Translate.setStatus("progress", processed, total, totalTokens)

    local startIndex = 1

    local function finalize()
        local finalTokens = tokenBase + totalTokens
        state.translate.totalTokens = finalTokens
        Translate.applyTranslateSuccessStatus(summaryEntries, finalTokens, true)
        state.translate.busy = false
        Translate.setControlsEnabled(true)
        if type(extraOnComplete) == "function" then
            extraOnComplete()
        end
    end

    local function handleProgress(task, result)
        if result and result.success then
            totalTokens = totalTokens + (result.tokens or 0)
        end
        if type(onTaskResult) == "function" then
            onTaskResult(task, result)
        end
        local count = tonumber(task.processCount) or 1
        processed = processed + count
        Translate.setStatus("progress", processed, total, totalTokens)
    end

    local function launchBatch()
        if startIndex > #list then
            finalize()
            return
        end
        local batchEnd = math.min(#list, startIndex + concurrency - 1)
        local tasks = {}
        for pos = startIndex, batchEnd do
            local item = list[pos]
            local task, taskErr = buildTask(item, pos, list)
            if not task then
                local count = 1
                if type(item) == "table" and type(item.indices) == "table" then
                    count = #item.indices
                end
                local fallbackTask = {
                    index = pos,
                ...
                if type(item) == "number" and entries then
                    local entry = entries[item]
                    fallbackTask.index = item
                    fallbackTask.entry = entry
                    fallbackTask.rowIndex = (entry and entry.index) or item
                elseif type(item) == "table" then
                    fallbackTask.group = item
                end
                handleProgress(fallbackTask, { success = false, err = taskErr or "payload_build_failed" })
            else
                if task.processCount == nil then
                    task.processCount = 1
                end
                tasks[#tasks + 1] = task
            end
        ...
        local started, startErr = AsyncTranslation.launchBackgroundCurl(tasks, {
            apiUrl = apiUrl,
            timeout = timeout,
        ...
        if not started then
            local errResult = { success = false, err = startErr or "parallel_execution_failed" }
            for _, task in ipairs(tasks) do
                handleProgress(task, errResult)
            end
            startIndex = batchEnd + 1
            launchBatch()
        end
...
function ChatProviders.translateEntries(configOrId, entries, targetLabel, options)
    local config = ChatProviders.getConfig(configOrId)
    if not config then
    ...
    local payloadPrefix = config.parallelPayloadPrefix or string.format("%s_payload", config.id or "chat")
    local outputPrefix = config.parallelOutputPrefix or string.format("%s_output", config.id or "chat")
    ...
    return Translate.runBatchRequests({
        ...
        maxRetry = config.maxRetry or GLM_MAX_RETRY,
        ...
        buildTask = function(idx)
            local entry = entries[idx]
            local prefixText, suffixText = Translate.buildTranslateContext(entries, idx)
            return {
                index = idx,
        ...
        onTaskResult = function(task, result)
            Translate.applyEntryResult(task, result, config.label)
        end,
    })
end
...
function DeepL.translateEntries(entries, targetLabel, options)
    ...
    local _, targetCode, targetErr = Translate.resolveTargetSelection(TRANSLATE_PROVIDER_DEEPL_LABEL, targetLabel)
    ...
    return Translate.runBatchRequests({
        ...
        onComplete = options.onComplete,
        summaryEntries = options.summaryEntries or entries,
        providerLabel = TRANSLATE_PROVIDER_DEEPL_LABEL,
        buildTask = function(idx)
            local entry = entries[idx]
            return {
                index = idx,
...
function Azure.resolveCredential()
    ...
    local apiKey, err = Translate.fetch_provider_secret(AZURE_SUPABASE_PROVIDER)
    if not apiKey then
        return nil, nil, err
    end
...
function OpenAIService.requestSmartMerge(sources, targetLabel, config, timeout)
    ...
    function Azure.translateEntries(entries, targetLabel, options)
        ...
        local _, targetCode, targetErr = Translate.resolveTargetSelection(TRANSLATE_PROVIDER_AZURE_LABEL, targetLabel)
        if not targetCode then
            return nil, targetErr or "missing_target_lang"
        end
        ...
        return Translate.runBatchRequests({
            ...
            onComplete = options.onComplete,
            summaryEntries = options.summaryEntries or entries,
            providerLabel = TRANSLATE_PROVIDER_AZURE_LABEL,
            buildTask = function(idx)
                local entry = entries[idx]
                return {
                    index = idx,
    ...
    function OpenAIService.translateEntries(entries, targetLabel, options)
        if not entries or #entries == 0 then
            return nil, "no_entries"
        end
        ...
        return Translate.runBatchRequests({
            ...
            onComplete = options.onComplete,
            summaryEntries = options.summaryEntries or entries,
            providerLabel = TRANSLATE_PROVIDER_OPENAI_LABEL,
            buildTask = function(idx)
                local entry = entries[idx]
                local prefixText, suffixText = Translate.buildTranslateContext(entries, idx)
                return {
...
function Translate.resolveProviderAdapter(provider)
    local spec = Translate.getProviderSpec(provider)
    if not spec then
        return nil, "provider_not_supported"
    end

    if spec.kind == "azure" then
        local adapter = {
            label = spec.label,
            langKey = spec.langKey,
            smartMergeMode = spec.smartMergeMode,
            allowParallel = true,
            translateEntries = Azure.translateEntries,
        }
        adapter.buildSmartMergeContext = function(targetLabel, targetCode)
            ...
            local baseUrl = Utils.trim((state.azure and state.azure.baseUrl) or AZURE_DEFAULT_BASE_URL)
            if baseUrl == "" then
                baseUrl = AZURE_DEFAULT_BASE_URL
            end
            local cleanBase = Utils.trim(baseUrl)
            if cleanBase == "" then
                cleanBase = AZURE_DEFAULT_BASE_URL
            end
            cleanBase = cleanBase:gsub("/+$", "")
            local apiUrl = cleanBase .. "/translate" .. string.format("?api-version=3.0&to=%s", Utils.urlEncode(targetCode))
            return {
                apiUrl = apiUrl,
                headers = {
                    string.format("Ocp-Apim-Subscription-Key: %s", apiKey),
                    string.format("Ocp-Apim-Subscription-Region: %s", (region and region ~= "" and region) or AZURE_FALLBACK_REGION),
                    "Content-Type: application/json",
                    string.format("User-Agent: %s/%s", Config.SCRIPT_NAME, Config.SCRIPT_VERSION),
                },
                parser = Azure.parseResponseBody,
                buildPayload = function(group)
                    return json.encode({
                        { text = table.concat(group.sources, "") },
                    })
                end,
        ...
        return adapter
    elseif spec.kind == "deepl" then
        local adapter = {
            label = spec.label,
            langKey = spec.langKey,
            smartMergeMode = spec.smartMergeMode,
            allowParallel = true,
            translateEntries = DeepL.translateEntries,
        }
        adapter.buildSmartMergeContext = function(targetLabel, targetCode)
            if spec.smartMergeMode ~= "ratio" then
                return nil, "provider_not_supported"
            ...
            return {
                apiUrl = DeepL.resolveApiUrl(apiKey),
                headers = {
                    string.format("Authorization: DeepL-Auth-Key %s", apiKey),
                    "Content-Type: application/json",
                    string.format("User-Agent: %s/%s", Config.SCRIPT_NAME, Config.SCRIPT_VERSION),
                },
                parser = DeepL.parseResponseBody,
                buildPayload = function(group)
                    return json.encode({
                        text = { table.concat(group.sources, "") },
                        target_lang = targetCode,
                ...
                payloadPrefix = "smartmerge_deepl_payload",
                outputPrefix = "smartmerge_deepl_output",
                splitMode = "ratio",
            }
        end
        return adapter
    elseif spec.kind == "openai" then
        local adapter = {
            label = spec.label,
            langKey = spec.langKey,
            smartMergeMode = spec.smartMergeMode,
            allowParallel = true,
            translateEntries = OpenAIService.translateEntries,
        }
        adapter.buildSmartMergeContext = function(targetLabel, targetCode)
            if spec.smartMergeMode ~= "llm" then
                return nil, "provider_not_supported"
            end
            OpenAIService.applyConfigFromControls()
            Storage.ensureOpenAIModelList(state.openaiFormat)
            ...
            return {
                ...
                parser = OpenAIService.parseResponseBody,
                buildPayload = function(group)
                    return json.encode({
                        model = selected.name,
                        messages = Translate.buildSmartMergeMessages(targetLabel, group.sources or {}),
                        temperature = temperature,
                    })
                end,
                payloadPrefix = "smartmerge_openai_payload",
                outputPrefix = "smartmerge_openai_output",
                splitMode = "llm",
            }
        end
        return adapter
    elseif spec.kind == "chat" then
        ...
        local adapter = {
            ...
            langKey = spec.langKey,
            smartMergeMode = spec.smartMergeMode,
            allowParallel = providerConfig.allowParallel ~= false,
            translateEntries = function(entries, targetLabel, options)
                return ChatProviders.translateEntries(providerConfig, entries, targetLabel, options)
            end,
        }
        adapter.buildSmartMergeContext = function(targetLabel, targetCode)
            if spec.smartMergeMode ~= "llm" then
            ...
            local id = providerConfig.id or "chat"
            return {
                apiUrl = providerConfig.apiUrl,
                headers = ChatProviders.buildHeaderList(providerConfig, apiKey),
                parser = providerConfig.parseResponse or ChatProviders.parseResponseBody,
                buildPayload = function(group)
                    return ChatProviders.buildSmartMergePayload(providerConfig, group.sources, targetLabel) or ""
                end,
                payloadPrefix = string.format("smartmerge_%s_payload", id),
                outputPrefix = string.format("smartmerge_%s_output", id),
                splitMode = "llm",
            }
        end
        return adapter
    ...
    local function applySmartMergeGroup(entries, group, translations, errMsg, providerLabel)
        if translations then
            for i, idx in ipairs(group.indices) do
                local text = translations[i] or ""
                if entries[idx] then
                    entries[idx].translation = text
                end
                Translate.updateTranslateTreeRow(idx, text)
                Translate.clearTranslateFailure(idx)
            end
        else
            local normalized = Translate.normalizeError(errMsg, "translation_failed", providerLabel)
            local fallback = Translate.formatTranslateFailure(normalized, providerLabel)
            for _, idx in ipairs(group.indices) do
                if entries[idx] then
                    entries[idx].translation = fallback
                end
                Translate.updateTranslateTreeRow(idx, fallback)
                Translate.markTranslateFailure(idx, normalized.code)
            end
        end
    end

local function translateEntriesSmartMerge(entries, adapter, targetLabel, indices, options)
        if not entries or #entries == 0 then
            return nil, "no_entries"
        end
        if not adapter or not adapter.label then
            return nil, "provider_not_supported"
        end
        options = options or {}
        local list = indices
        ...
        local provider = adapter.label
        local resolvedLabel, targetCode, targetErr = Translate.resolveTargetSelection(provider, targetLabel)
        if not resolvedLabel then
            return nil, targetErr or "missing_target_lang"
        end
        targetLabel = resolvedLabel

        local groups = Translate.mergeSubtitleChunks(entries, list)
        if #groups == 0 then
            return nil, "no_entries"
        end
        local context
        local contextErr
        if type(adapter.buildSmartMergeContext) == "function" then
            context, contextErr = adapter.buildSmartMergeContext(targetLabel, targetCode)
        end
        if not context then
            return nil, contextErr or "provider_not_supported"
        end

        local apiUrl = context.apiUrl
        local headers = context.headers
        local parser = context.parser
        local buildPayload = context.buildPayload
        local payloadPrefix = context.payloadPrefix or "smartmerge_payload"
        local outputPrefix = context.outputPrefix or "smartmerge_output"
        local splitMode = context.splitMode or adapter.smartMergeMode

        if not (apiUrl and headers and parser and buildPayload) then
            return nil, "provider_not_supported"
        end

        local totalEntries = #list
        local providerLabel = adapter.label
        ...
        return Translate.runBatchRequests({
            ...
            onComplete = options.onComplete,
            summaryEntries = options.summaryEntries or entries,
            allowParallel = adapter.allowParallel ~= false,
            providerLabel = providerLabel,
            buildTask = function(group, pos)
                return {
                    index = pos,
                    groupIndex = pos,
                    group = group,
                    processCount = #group.indices,
                    payload = buildPayload(group),
                    url = apiUrl,
                }
            end,
            onTaskResult = function(task, result)
                local group = task.group or groups[task.groupIndex or task.index]
                if not group then
                    return
                end
                local translations
                local errMsg
                if result and result.success then
                    if splitMode == "ratio" then
                        translations = Translate.splitTranslationByRatio(result.translation, group.sources)
                    else
                        translations, errMsg = Translate.parseSmartMergeJson(result.translation, group.chunkCount)
                    end
                    if not translations then
                        errMsg = errMsg or result.err or "translation_failed"
                    end
                else
                    errMsg = (result and result.err) or "translation_failed"
                end

                if translations then
                    applySmartMergeGroup(entries, group, translations, nil, providerLabel)
                else
                    applySmartMergeGroup(entries, group, nil, errMsg, providerLabel)
                end
            end,
        })
    end

    function Translate.performWorkflow()
        if state.translate.busy then
            return false
        end

        Translate.setStatus("preparing")
        OpenAIService.applyConfigFromControls()

        Translate.ensureTranslateEntries(false)
        local entries = state.translate.entries or {}
        if #entries == 0 then
        ...
        state.translate.provider = provider
        local adapter, adapterErr = Translate.resolveProviderAdapter(provider)
        if not adapter then
            Translate.reportError(adapterErr or "provider_not_supported", { showMessage = true, provider = provider })
            return false
        end
        local translateFunc = adapter.translateEntries

        local targetLabel = it.TranslateTargetCombo and it.TranslateTargetCombo.CurrentText or state.translate.targetLabel
        local resolvedLabel, _, targetErr = Translate.resolveTargetSelection(provider, targetLabel)
        if not resolvedLabel then
        ...
        local useSmartMerge = state.translate and state.translate.smartMerge == true
        local ok, totalTokens, err
        if useSmartMerge then
            ok, totalTokens, err = pcall(translateEntriesSmartMerge, entries, adapter, targetLabel)
        else
            ok, totalTokens, err = pcall(translateFunc, entries, targetLabel)
        end

        local asyncActive = AsyncTranslation and AsyncTranslation.isActive and AsyncTranslation.isActive()
        if asyncActive then
            return true
        end
        ...
        if not ok then
            ...
            Translate.reportError(totalTokens or "translation_failed", { showMessage = true, provider = provider })
            return false
        end

        if not totalTokens then
            state.translate.totalTokens = 0
            Translate.reportError(err or "translation_failed", { showMessage = true, provider = provider })
            return false
        end

        state.translate.totalTokens = totalTokens
        Translate.applyTranslateSuccessStatus(entries, totalTokens, true)
        return true
    end

    function Translate.translateSingleEntry(index, targetLabel)
        OpenAIService.applyConfigFromControls()
        local entries = state.translate.entries or {}
        local entry = entries[index]
        if not entry then
            Translate.reportError("no_selection")
            return false
        end
        local provider = state.translate.provider or TRANSLATE_PROVIDER_AZURE_LABEL
        local resolvedLabel, _, targetErr = Translate.resolveTargetSelection(provider, targetLabel)
        if not resolvedLabel then
            Translate.reportError(targetErr or "missing_target_lang", { provider = provider })
            return false
        end
        targetLabel = resolvedLabel
        local tokenBase = state.translate.totalTokens or 0
        local summaryEntries = { entry }

        local adapter, adapterErr = Translate.resolveProviderAdapter(provider)
        if not adapter then
            Translate.reportError(adapterErr or "provider_not_supported", { provider = provider })
            return false
        end

        if state.translate and state.translate.smartMerge then
            local totalTokens, err = translateEntriesSmartMerge(entries, adapter, targetLabel, { index }, {
                tokenBase = tokenBase,
                summaryEntries = summaryEntries,
            })
            if not totalTokens then
                Translate.reportError(err or "translation_failed", { provider = provider })
                return false
            end
            return true
        end

        local totalTokens, err = adapter.translateEntries(entries, targetLabel, {
            indices = { index },
            tokenBase = tokenBase,
            summaryEntries = summaryEntries,
        })
    ...
    function Translate.retryFailedEntries()
        if state.translate.busy then
            return false
        end
        ...
        local ok, totalTokens, err
        if useSmartMerge then
            ok, totalTokens, err = pcall(translateEntriesSmartMerge, entries, adapter, targetLabel, indices, {
                tokenBase = tokenBase,
                onComplete = finalizeRetry,
                summaryEntries = subset,
            })
        else
            ok, totalTokens, err = pcall(adapter.translateEntries, subset, targetLabel, {
                tokenBase = tokenBase,
                onComplete = finalizeRetry,
            })
        end
        ...
        if not ok then
            local normalized = Translate.normalizeError(totalTokens or "translation_failed", "translation_failed", provider)
            local reasonKey = normalized.code
            local en, cn = Translate.resolveErrorPair(reasonKey)
            Translate.notifyTranslateStatus("failed", { en }, { cn })
            Translate.setStatus("failed", Translate.resolveError(reasonKey))
            Translate.updateRetryButtonState()
            return false
        end

        if not totalTokens then
            local normalized = Translate.normalizeError(err or "translation_failed", "translation_failed", provider)
            local reasonKey = normalized.code
            local en, cn = Translate.resolveErrorPair(reasonKey)
            Translate.notifyTranslateStatus("failed", { en }, { cn })
            Translate.setStatus("failed", Translate.resolveError(reasonKey))
        ...
        Translate.applyTranslateSuccessStatus(subset, state.translate.totalTokens, true)
        Translate.updateRetryButtonState()
        return true
    end
...
function Subtitle.refreshFromTimeline()
    ...
    if not ok then
        ...
        UI.updatePlayheadTimerState(false)
        return false
    end
    Translate.resetState()
    UI.populateTree()
...
function App.checkForUpdates()
    local ok, result = pcall(Services.supabaseCheckUpdate, Config.SCRIPT_NAME)
    if not ok then
...
function UI.clearTreeSelectionNodes(widget)
...
function UI.selectTreeNode(widget, index, opts)
    ...
    if opts.clear ~= false then
        UI.clearTreeSelectionNodes(widget)
    end
    item.Selected = true
...
function UI.clearTreeSelection()
    UI.clearTreeSelectionNodes(it.SubtitleTree)
end

function UI.cloneOptionTable(src)
...
function UI.focusEntryRow(index, opts)
    ...
    if includeEdit and it.SubtitleTree then
        ...
        UI.selectTreeNode(it.SubtitleTree, index, { scroll = scrollEdit, clear = clearEdit })
    ...
    if includeTranslate and tState and tState.entries then
        ...
        if tEntry then
            ...
            if it.TranslateSubtitleTree then
                UI.selectTreeNode(it.TranslateSubtitleTree, index, { scroll = scrollTranslate, clear = clearTranslate })
            end
...
function Translate.selectRow(index, opts)
    ...
    elseif type(opts) == "table" then
        config = UI.cloneOptionTable(opts)
    ...
    UI.focusEntryRow(index, config)
end

-- 修改：仅选中当前命中的条目
function UI.jumpToEntry(index, doTimeline, opts)
    local config = UI.cloneOptionTable(opts)
    ...
    return UI.focusEntryRow(index, config)
end
    ...
    for index, entry in ipairs(state.entries) do
        ...
        if changes > 0 then
            ...
            if state.translate and state.translate.entries and state.translate.entries[index] then
                ...
                Translate.updateTranslateOriginalRow(index, newText)
            end
            UI.setRowHighlight(index, findHighlightColor)
...
function win.On.MainTabs.CurrentChanged(ev)
    ...
    if index == 1 then
        ...
        if not (t and t.populated and t.entries and #t.entries > 0) then
            Translate.ensureTranslateEntries(true)  
        end
...
function win.On.DonationButton.Clicked(ev)
    if UI.currentLanguage() == "cn" then
        Utils.openExternalUrl(Config.SCRIPT_TAOBAO_URL)
    else
        Utils.openExternalUrl(Config.SCRIPT_KOFI_URL)
    end
end
...
function win.On.CopyrightButton.Clicked(ev)
    ...
    local targetUrl = preferEnglish and Config.SCRIPT_KOFI_URL or Config.SCRIPT_TAOBAO_URL
...
function win.On.TranslateProviderCombo.CurrentIndexChanged(ev)
    if it.TranslateProviderCombo then
        ...
        state.translate.provider = provider
        Translate.refreshTranslateTargetCombo(provider, state.translate.targetLabel, state.translate.targetCode)
    end
end
...
function win.On.TranslateConcurrencyCombo.CurrentIndexChanged(ev)
    ...
    if not index or index < 0 then
        ...
        Translate.setConcurrencyComboSelection(state.translate.concurrency)
        return
    end
...
function win.On.TranslateTargetCombo.CurrentIndexChanged(ev)
    if it.TranslateTargetCombo then
        ...
        state.translate.targetLabel = it.TranslateTargetCombo.CurrentText
        state.translate.targetCode = Translate.getTargetCodeFromIndex(provider, index)
    end
end
function win.On.TranslateSmartMergeCheck.Clicked(ev)
    if not it.TranslateSmartMergeCheck then
        return
    end
    state.translate.smartMerge = it.TranslateSmartMergeCheck.Checked == true
    if state.translate.smartMerge then
        UI.showMessage(
            "Merge multiple subtitle blocks into full-sentence translation for better context; ideal for vertical videos. Tip: works best when subtitle blocks end with punctuation (.,!,? etc.).",
            "将多个字幕合并为完整句子翻译，提供足够上下文，适合竖屏视频，tips：字幕块句末有标点符号（。！？等）效果最好"
        )
    end
end
