...
function App:Run()
    ...
    if UI.MainWin and UI.MainWin.On then
        ...
        UI.MainWin.On.MainTabs.CurrentChanged = function(ev)
            ...
            if idx == TAB_INDEX_EDIT then
                if UI.generatedImagePath and UI.generatedImagePath ~= "" then
                    -- 将生成的图片加载到第一个空位（通常是空的）
                    local loaded = false
                    for i = 1, 4 do
                        if not UI.currentImagePaths[i] then
                            if loadImageToSlot then
                                loadImageToSlot(UI.generatedImagePath, i)
                            else
                                UI.currentImagePaths[i] = UI.generatedImagePath
                            end
                            loaded = true
                            break
                        end
        ...
        UI.MainWin.On.GenerateBtn.Clicked = function()
            ...
            for i = 1, 4 do
                if UI.currentImagePaths[i] and UI.currentImagePaths[i] ~= "" then
                    Utils.debugKV("Image Path " .. i, UI.currentImagePaths[i])
                end
            end
            ...
            for i = 1, 4 do
                if UI.currentImagePaths[i] and UI.currentImagePaths[i] ~= "" then
                    local dataUrl = Utils.encodeImageToBase64(UI.currentImagePaths[i])
                    if dataUrl then
                        table.insert(imageDataUrls, dataUrl)
                        Utils.debugSection("IMAGE ENCODED " .. i)
                        Utils.debugKV("Path", UI.currentImagePaths[i])
                        Utils.debugKV("Data URL Length", tostring(#dataUrl))
                        Utils.debugKV("Preview", Utils.truncate(dataUrl, 120))
                        Utils.debugSectionEnd()
                    else
                        print("Warning: Failed to encode image " .. i .. ": " .. UI.currentImagePaths[i])
                    end
                end
            end
            ...
            local function call_google_provider(promptText, dataUrls, providerId, saveDir)
                local pid = providerId or (UI.config.provider_choice or "google")
            ...
            local function call_seed_provider(promptText, dataUrl, aspectData)
	            ...
	            if providerChoice == "seeddream" then
	                imageSource, providerMeta = call_seed_provider(prompt, imageDataUrls[1], seedAspectData)
	            else
	                imageSource, providerMeta = call_google_provider(prompt, imageDataUrls, providerChoice, saveDir)
	            end
	            if not imageSource then
