    ...
    local Locale = {
        ...
        strings = {
            en = {
                ...
                donation_button = "☕ Explore Script ☕",
                ...
                log_need_two_markers = "At least two markers are required to build segments.",
            ...
            zh = {
                ...
                log_need_two_markers = "至少需要两个 Mark 才能生成区间。",
...
do
    ...
    function Logger:_update_view()
        local items = App.UI and App.UI.items
        if items and items.LogView then
            items.LogView.Text = table.concat(self.lines, "\n")
        end
    end
...
do
    ...
    function Timeline:add_markers(interval_seconds, count)
        ...
        local num = (fpsFraction and fpsFraction.num) or 24
        local den = (fpsFraction and fpsFraction.den) or 1
        if den == 0 then
            den = 1
        end

        local interval_frames = math.max(1, math.floor((interval_seconds * num) / den + 0.5))
        local start_frame = 0

        for i = 0, count - 1 do
            local frame = start_frame + i * interval_frames
            local name = string.format("Batch_%03d", i + 1)
            local ok = timeline:AddMarker(frame, App.Config.MARK_COLOR, name, "", 1, App.Config.MARK_TAG)
            if not ok then
    ...
    function Timeline:build_segments()
        ...
        if #sorted < 2 then
            App.Logger:append_localized("log_need_two_markers")
            return nil
        end
        local fpsFraction = App.Helpers:get_timeline_fps_fraction(timeline)
        ...
        for i = 1, #sorted - 1 do
            local startFrame = math.floor(sorted[i].frame + 0.5)
            local endFrame = math.floor(sorted[i + 1].frame + 0.5)
            if endFrame > startFrame then
                ...
                if not has_media_between(trackCollections.video, startFrame, endFrame, timelineStartFrame) then
                    ...
                    local row = {
                        ...
                        endFrame = endFrame,
                        startTimecode = App.Helpers:frames_to_timecode_precise(displayStartFrame, fpsFraction),
...
do
    ...
    local function attempt_start_rendering(project, jobIds)
        local attempts = {}
        local function attempt(label, fn)
            ...
            local reason = ok and "false" or ("error: " .. tostring(result))
            attempts[#attempts + 1] = string.format("%s -> %s", label, reason)
            return false
        end

        local ok, label = attempt("list+interactive", function()
            return project:StartRendering(jobIds, true)
        end)
        if ok then return true, label end

        ok, label = attempt("list", function()
            return project:StartRendering(jobIds)
        end)
        if ok then return true, label end

        if unpack_list and jobIds and #jobIds > 0 then
            ok, label = attempt("varargs", function()
                return project:StartRendering(unpack_list(jobIds))
            end)
            if ok then return true, label end
        end

        if jobIds and #jobIds == 1 then
            local single = jobIds[1]
            ok, label = attempt("single+interactive", function()
                return project:StartRendering(single, true)
            end)
            if ok then return true, label end

            ok, label = attempt("single", function()
                return project:StartRendering(single)
            end)
            if ok then return true, label end
        end

        ok, label = attempt("all-jobs", function()
    ...
    local function delete_render_job(project, jobId)
        if not (project and jobId and project.DeleteRenderJob) then
            return
        end
        pcall(function()
    ...
    function Render:start_next_job()
        ...
        seq.currentIndex = (seq.currentIndex or 0) + 1
        local row = seq.rows[seq.currentIndex]
        if not row then
            self.sequence.active = false
            return false
        end

        local okEnv, envReason = ensure_render_env(project, seq.targetDir)
        if not okEnv then
            ...
            abort_sequence("log_start_render_failed")
            return false
        end

        local _, keyChanged = update_row_status(row, "waiting", nil)
        if keyChanged then
            log_row_status(row, "waiting")
        end

        local settings = build_settings(row, seq.targetDir, seq.baseName)
        local ok = project:SetRenderSettings(settings)
        if not ok then
            App.Logger:append_localized("log_render_settings_failed", row.name or ("#" .. tostring(seq.currentIndex)))
        ...
        if not jobId then
            App.Logger:append_localized("log_add_job_failed", row.name or ("#" .. tostring(seq.currentIndex)))
            update_row_status(row, "failed", nil)
            log_row_status(row, "failed")
            abort_sequence("log_start_render_failed")
            return false
        ...
        App.State:set_job_ids({ jobId })
        App.Logger:append_localized("log_job_added", row.name or jobId)

        local started, detail = attempt_start_rendering(project, { jobId })
        if not started then
            delete_render_job(project, jobId)
            App.Logger:append_localized("log_start_job_failed", row.name or jobId)
            if type(detail) == "table" and #detail > 0 then
                App.Logger:append_localized("log_attempts", table.concat(detail, " | "))
            end
            update_row_status(row, "failed", nil)
            log_row_status(row, "failed")
            abort_sequence("log_start_render_failed")
        ...
        self.monitor.runningJobId = jobId
        self.monitor.runningMode = detail
        update_row_status(row, "rendering", nil)
        ensure_ui_refresh()
        App.Logger:append_localized("log_job_started", row.name or jobId)
        return true
    end
    ...
    function Render:enqueue_rows(rows)
        ...
        for _, row in ipairs(rows) do
            row.jobId = nil
            row.statusExtra = nil
            local _, keyChanged = update_row_status(row, "queued", nil)
            if keyChanged then
    ...
    function Render:poll_job_status()
        ...
        local runningJobId = self.monitor.runningJobId
        if not runningJobId then
            if self.sequence and self.sequence.active then
                local started = self:start_next_job()
                if not started then
                    complete_sequence()
                end
            else
        ...
        local row = App.State:get_row_by_job(runningJobId)
        local statusInfo
        local ok, result = pcall(function()
            return project:GetRenderJobStatus(runningJobId)
        end)
        if ok then
            statusInfo = result
        end
        local statusKey, progressText = normalize_job_status(statusInfo)
        if statusKey == "unknown" and row then
            statusKey = row.statusKey or "unknown"
            progressText = row.statusExtra
        end
        if row then
        ...
        if statusKey == "done" then
            delete_render_job(project, runningJobId)
            self.monitor.runningJobId = nil
            local started = self:start_next_job()
            if not started then
                complete_sequence()
        ...
        elseif statusKey == "failed" then
            delete_render_job(project, runningJobId)
            App.Logger:append_localized("log_job_failed_cancel_rest", row and row.name or tostring(runningJobId))
            abort_sequence("log_detected_failure_stop")
        elseif not project:IsRenderingInProgress() then
            delete_render_job(project, runningJobId)
